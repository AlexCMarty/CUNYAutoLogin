...
4. **`browser.cookies` today** — Only **`cookies.remove`** is used, on a **fixed pair of Brightspace session names** with `url: BRIGHTSPACE_HOME_URL` (`src/background/service-worker.ts`). There are **no** `cookies.get` / `cookies.set` calls and no persistence of cookie **values**. If you add cookie APIs later: **`cookies.remove` only** on the **smallest** documented set; never **`cookies.set`** or jar carpet-bombing without explicit human verification. Add **`"cookies"`** permission and matching **`host_permissions`** — lack of permission is **not** an excuse for content-script `document.cookie` writes or remote cookie injection; use **documentation / user steps** instead.
