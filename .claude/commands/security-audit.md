...
- **`src/sidebar/sidebar.ts`** — the sidebar entrypoint async-loads `onboarding/render.ts` when there is no vault, when a session resume snapshot exists, or when the URL hash contains `#onboarding=1` (dev/e2e `import.meta.env.MODE` only); otherwise loads `sidebar/vaultController.ts` for vault management. Confirm the `#onboarding=1` branch cannot activate in production builds.
