...
| v1 | v2 |
|---|---|
| Headline is a promise: "Never type your CUNY password again" | Headline is a factual description; trust signal ("stored only on this device") gets equal visual weight |
| Screen 2 label: "What's your CUNYfirst email?" | Screen 2 label: "What email do you use to log in to Brightspace?" |
| Screen 3 has no acknowledgment after credentials are entered | Reassurance line added: "We'll use these to log you in once, right now, so you can watch it work." |
| Screen 4 gives no direction on where to look | Directional line added: "The action is on the new tab — this window will update automatically." |
| Screen 4 pulsing animation is unexplained | Label added beneath animation: "Nothing to do yet — waiting for the tab to open." |
| Screen 4-error banner is indistinct from CUNY's own alerts | Banner must include extension icon and be visually distinct from CUNY error styling |
| Screen 5 body: "CUNY is asking for permission to continue" | Screen 5 body explains what Allow does: confirming identity before showing account settings |
| Screens 6–9 popup shows one static message for all four steps | Popup shows a step-specific "why" line for each guided step |
| Screen 8 gives no warning that a silent capture is coming | Screen 8 popup primes student: "On the next step, we'll automatically save the key…" |
| Screen 10 first failure immediately explains device clock | Clock explanation moved to second failure only; first failure is just "wait and try again" |
